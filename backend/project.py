        ...
        async with httpx.AsyncClient() as client:
            ...
            if "_embedded" in data and "elements" in data["_embedded"]:
                ...
                if all_projects:
                    ...
                    raise HTTPException(
                        status_code=404, detail="No projects found.")
            else:
                raise HTTPException(
                    status_code=404, detail="Invalid response format.")

    except requests.exceptions.RequestException as e:
        raise HTTPException(
            status_code=500, detail="Failed to connect to the API.")
    except ValueError as e:
        raise HTTPException(
            status_code=500, detail="Invalid JSON response from the API.")
...
async def project_count_by_status():
    all_projects = await get_all_projects()
    status_list = [project.get("project_status")
                    for project in all_projects if project.get("project_status")]
...
async def project_count_by_priority():
    all_projects = await get_all_projects()
    priority_list = [project.get("project_priority")
                    for project in all_projects if project.get("project_priority")]
...
async def get_progress_assignee_project(wp_types=None):
    ...
    for project_name, progress in assignee_progress.items():
        for data in progress:
            data["progress"] = round(
                (data["wp_done"] / data["wp_total"]) * 100)
        result.append({
            "project_name": project_name,
            "progress": progress
        })
