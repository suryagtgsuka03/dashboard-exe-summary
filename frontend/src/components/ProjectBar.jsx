...
function ProjectBar() {
  ...
  const fetchChartData = async (selectedTypeValues, selectedVersionValue) => {
    try {
      ...
      const filteredProject = data.find(
        (project) => project.project_name === getLabelFromURL()
      );
      if (filteredProject) {
        if (
          selectedVersionValue === "all" ||
          selectedVersionValue === "all-versions"
        ) {
  ...
  const fetchAssigneeChartData = async (
    selectedTypeValues,
    ...
    try {
      ...
      if (selectedVersionValue === "all-versions") {
        response = await axios.get(
          `${BASE_URL}/get-progress-assignee-project`,
          {
            params: {
        ...
        if (selectedTypeValues.includes("all")) {
          ...
          for (const typeOption of typeOptions) {
            if (typeOption.value !== "all") {
              response = await axios.get(
                `${BASE_URL}/get-progress-assignee-version`,
                {
                  params: {
      ...
      if (filteredProject) {
        if (selectedVersionValue === "all-versions") {
          assigneeChartData = {
            labels: filteredProject.progress.map(
              (progress) => progress.user_name
            ),
            datasets: [
              {
          ...
          if (selectedVersionData) {
            assigneeChartData = {
              labels: selectedVersionData.progress.map(
                (progress) => progress.member_name
              ),
              datasets: [
                {
                  label: selectedMetric,
  ...
  const fetchVersionOptions = async (selectedTypeValues) => {
    try {
      let response;
      if (
        selectedTypeValues.includes("all") ||
  ...
  useEffect(() => {
    ...
    const isAllVersionsSelected =
      selectedVersion && selectedVersion.value === "all-versions";

    if (
      isAllVersionsSelected ||
      selectedTypes.some((option) => option.value === "all")
    ) {
  ...
  const handleTypeSelect = (selectedOptions) => {
    ...
    const isAllTypesSelected = selectedOptions.some(
  ...
  return (
    <Container fluid className="project-components">
      ...
      <Row className="container-chart">
        <h3 className="sub-judul-project">Sub Project</h3>
          <SubProjectBar />
      </Row>
      <Row className="container-chart">
